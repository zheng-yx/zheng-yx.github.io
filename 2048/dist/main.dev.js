...
$(function () {
  ...
  function refreshGame() {
    ...
    $('#gameScore').html(gameScore); //随机生成两个新元素
  ...
  function getSideItem(currentItem, direction) {
    ...
    var currentItemY = currentItem.attr('y') - 0; //根据方向获取旁边元素的位置
  ...
  function itemMove(currentItem, direction) {
    ...
    if (sideItem.length == 0) {//当前元素在最边上
      ...
      $('#gameScore').html(gameScore); // itemMove(sideItem, direction);
  ...
  function move(direction) {
    //获取所有非空元素
    var nonEmptyItems = $('.gameBody .row .nonEmptyItem'); //如果按下的方向是左或上，则正向遍历非空元素
  ...
  function gameInit() {
    //初始化分数
    $('#gameScore').html(gameScore); //为刷新按钮绑定事件

    $('.refreshBtn').click(refreshGame); //随机生成两个新元素
  ...
  function newRndItem() {
    ...
    console.log('newRndNum: ' + newRndNum); //随机生成新数字的位置
  ...
  (function () {
    ...
    function mobilwmtouch(obj) {
      ...
      obj.addEventListener("touchstart", function (e) {
        ...
        stouy = e.targetTouches[0].clientY; //console.log(stoux);
      }, false);
      obj.addEventListener("touchend", function (e) {
        ...
        xdire = etoux - stoux;
        ydire = etouy - stouy;
        chazhi = Math.abs(xdire) - Math.abs(ydire); //console.log(ydire);

        if (xdire > 0 && chazhi > 0) {
          console.log("right"); //alert(evenzc('touright',alerts));
